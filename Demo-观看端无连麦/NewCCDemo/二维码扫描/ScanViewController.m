...
-(void)parseCodeStr:(NSString *)result {
    ...
    if (!StrNotEmpty(result) || rangeRoomId.location == NSNotFound || rangeUserId.location == NSNotFound || (self.index == 3 && rangeLiveId.location == NSNotFound)) {
        ...
        if(self.index == 1) {
            ...
            if(rangeRecordId.location == NSNotFound) {
                ...
                NSString *userId = [result substringWithRange:NSMakeRange(rangeUserId.location + rangeUserId.length, rangeRecordId.location - 1 - (rangeUserId.location + rangeUserId.length))];
                NSString *recordId = [result substringWithRange:NSMakeRange(rangeRecordId.location + rangeRecordId.length, rangeLiveId.location - 1 - (rangeRecordId.location + rangeRecordId.length))];
                NSString *liveId = [result substringFromIndex:rangeLiveId.location + rangeLiveId.length];
                NSLog(@"roomId = %@,userId = %@,liveId = %@,recordId = %@",roomId,userId,liveId,recordId);
                SaveToUserDefaults(PLAYBACK_USERID,userId);
                SaveToUserDefaults(PLAYBACK_ROOMID,roomId);
                SaveToUserDefaults(PLAYBACK_LIVEID,liveId);
