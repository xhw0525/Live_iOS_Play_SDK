...
@property (strong, nonatomic) InformationShowView   *informationView;
...
-(void)dealPPTViewDoubleTap {
    ...
    if (!self.isScreenLandScape) {
        ...
        [UIView animateWithDuration:0.25f animations:^{
            ...
            [ws.requestDataPlayBack changeDocFrame:rect];
...
-(void)quanPingBtnClicked {
    ...
    if (!self.isScreenLandScape) {
        ...
        [[UIDevice currentDevice] setValue:[NSNumber numberWithInteger:UIDeviceOrientationPortrait] forKey:@"orientation"];
        [UIApplication sharedApplication].statusBarHidden = NO;
        [_requestDataPlayBack changePlayerFrame:_videoRect];
...
- (void)moviePlayBackStateDidChange:(NSNotification*)notification
{
    switch (_requestDataPlayBack.ijkPlayer.playbackState)
    {
        ...
        case IJKMPMoviePlaybackStatePaused: {
            if(_suspendButton.selected == YES && [_requestDataPlayBack isPlaying]) {
                [_requestDataPlayBack pausePlayer];
            }
